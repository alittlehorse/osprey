...
namespace  libserver{
    ...
    std::optional<r1cs_gg_ppzksnark_proof<default_r1cs_gg_ppzksnark_pp>> groth16_server::generate_proof() {
        typedef ram_tinyram<default_r1cs_gg_ppzksnark_pp> default_ram_with_pp;

        default_r1cs_gg_ppzksnark_pp::init_public_params();
        /* load everything */
        const ram_architecture_params<default_ram_with_pp> ap =  *generate_ram_architecture_params<default_ram_with_pp>("filename_path");

        auto bounds = *get_bounds("path");
        size_t tinyram_input_size_bound, tinyram_program_size_bound, time_bound;
        tinyram_input_size_bound = std::get<0>(bounds);
        tinyram_program_size_bound = std::get<1>(bounds);
        time_bound =  std::get<2>(bounds);

        tinyram_program program = *generate_program("path",ap);
    ...
    bool groth16_server::construct_proof1() {
        ...
        //typedef ram_tinyram<default_r1cs_gg_ppzksnark_pp> default_ram_with_pp;
        proof_public_params<default_r1cs_gg_ppzksnark_pp> _proof_public_params("avarage/avarage-processed_assembly.txt",
        ...
        auto ap = _proof_public_params.get_ram_architecture_params();
        auto bounds = _proof_public_params.get_bounds();
        log->write_log<size_t,size_t>("Will run on %zu register machine (word size = %zu)\n", ap.k, ap.w);
        ...
        auto program = _proof_public_params.get_tinyram_program();
        auto boot_trace = _proof_public_params.get_boot_trace();
