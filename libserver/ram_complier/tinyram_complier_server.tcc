...
namespace libserver {
    bool tinyram_complier_server::complie_tinyram(std::string &&file_path, std::string &&out_name) {
        ...
        if (getline(in, head)) {
            ...
            log->write_log(
                    "word size and register number parse processing success!\n word size is %d,\n register nunmber is %d \n",
                    w, k);
        } else {
        ...
        for (auto l:lines) {
            ...
            string s = libserver::string_helper::StrFormat<std::string, int, int, int, int>("%s %d %d %d %d", instr,
                                                                                            d["immflag"], d["des"],
                                                                                            d["arg1"], d["arg2"]);
    ...
    template<typename ppT>
    optional<tinyram_snark::r1cs_constraint_system<libff::Fr<ppT>>> tinyram_complier_server::complie_r1cs(
            const tinyram_snark::ram_architecture_params<tinyram_snark::ram_tinyram<ppT>> &ap,
            const unordered_map<const char *, size_t> &bounds) {
        log->write_log<size_t, size_t>("Will run on %zu register machine (word size = %zu)\n", ap.k, ap.w);
